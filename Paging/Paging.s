...
; Load segment registers with different descriptors

jmp paging
mov dword[0xb012],0xaaaaaa
excep_pnp:
	pop ebx                 ; pop the error code # equal to zero for page not present
	mov ebx, cr2            ; cr2 contains the linear address that created the exception
	shr ebx, 12             ; get the details of the page from the address
	and ebx, 0x3ff          ; ebx contains the offset of the page entry in f000
	bts dword [ebx*4], 0    ; make the page present bit 1
iret
...
;; Filling Page directory by the following 
;; filling 0000f017 from e000 to efff 
;; ds segment is from e000
;;	least significiant bits 
;;      1               7
;;	0(p) 0(q) 0(r) 1(a)   1(b) 0(c) 1(d) 1(e)
;; e: this bit is used for indicationg whether the requested page is present or not
;; d: this bit is for read/write rights, o-> only read rights, 1-> both read and write rights
;; c: this bit is for user / supervisor 0 -> page is assigned to supervisor privilege 1 -> assigned to user privilege level
;; b: 1 -> write-through caching is enabled for the associated page or page table
;; 	  0 -> write back caching is  enabled for the associated page or page table
;; a: 1 -> this bit says the caching of the associated page or page table is prevented
;;	  0 -> 	says the caching of the associated page or page table is enabled
mov eax, 0
mov ebx, 0x0000f017
pageDirectory:	
	mov [ds:eax], ebx
	...
	cmp eax,0x1000
	jne pageDirectory
;; now the ds segment is from f000
	mov ax,0x40
	mov ds,ax;
	mov ecx,0
	mov edx,0x0000b017
	mov eax,0  ;; 0 for even 1 for odd
;; odd number of page pointing to b000
;; even number of page pointing to c000
...
;; self mapping for a000,b000,c000,d000,e000,f000
mov dword [0xa*4], 0x0000a017
mov dword [0xc*4], 0x0000c017
mov dword [0xd*4], 0x0000d017
mov dword [0xe*4], 0x0000e017
mov dword [0xf*4], 0x0000f017
mov dword [0x378], 0x0000a017
mov dword [0x40], 0x0000b002
;mov dword[0x4028],0xa017
;mov dword[0x402c],0xb017
;;; now the ds segment is from f000
;	mov ax,0x40
;	mov ds,ax;
;	mov ecx,0
...
;
;
;; control register cr3 should have the address of the page directory
;; here the page directory address is e000
;; so we are moving the address to the cr3 register
;; for enabling paging we have to make 31 bit of control register cr0 to 1
pageEnable:
	mov eax,0xe000
	mov cr3,eax	;; moving the page directory address to the cr3 register
	mov eax,cr0
	bts eax,31  ;; making the 31 bit of the cr0 register 
	mov cr0,eax
	jmp foo
foo: nop
;; Question 1:
; setting a segmentdescriptor pointing to the far address
;    mov dword [es:0], 0xbabababa
;    mov dword [es:4], 0xbabababe
    mov dword [gs:0x5678], 0xbabecafe
    mov ebx, [0xd14]
    mov [es:0], ebx

;;Question 2
;; jump to next instruction with a huge offset
   jmp 0x50:dword label-code_base
    label: 
    mov dword [es:4], 0xbabababe
    mov dword [es:0], 0xbabababa
    mov dword [es:5], 0xabbbbbbb
	mov dword [ss:4], 0xfafafafa
    ; following instruction jumps back to the original code
    jmp 0x8:dword label2-code_base
    label2:

;; Q3
;; page fault exception 14 
    ; the page corresponding to fs has present bit 0
;    mov dword [fs:8], 0x0abbaada
jmp pageDisable


;; after completion of address translation we have to disable the paging 
;; so we have to make the 31 bit of cr0 register to 0
pageDisable:
	mov eax,cr0
	btr eax,31 ;; making the 31 bit of cr0 register to 0
	mov cr0,eax
