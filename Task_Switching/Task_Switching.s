...
; task 1 -pl 0  address = 0xa000

call 0x70:0
mov ax,0x18
mov es,ax
mov dword[es:0] , 0xaaaa1111
jmp chanHappy

align 0x100    ;;address = 0xa000     
;;task 4(pl 3) calls this procedure using a call gate
mov dword [0], 0xabcdabcd
retf



align 0x200	; address = 0xa200
; task 2   pl 1

call 0x79:0 ;;;call task 3

iret        ;;return to task1


align 0x200	; address = 0xa400
; task 3   pl2
call 0x82:0
iret


align 0x200	; address = 0xa600

; task 4    pl 3

call 0xba:0      ;;call a pl0 code via call gate

;;to create exeption
mov ax, 0x23      ;;pl-1 data segment of task2
mov fs, ax
;; creating General protection fault trying to access a pl-1 data segment 
mov dword [fs:0], 0xbabababa
iret


align 0x200	; address = 0xa800

; task 5 exeption handler pl 0

pop ebx  ;;ebx=0x20
add ebx, 0x2004
mov cx, 0x10
mov es, cx
mov dword[es:0x3008],ebx
bts dword [es:ebx], 14
mov edx, [es:ebx]
iret

align 0x200	; Address = 0xaa00
db 0x0
...
align 0x100	;address=0xb400
;TSS of interrupt handler
dd 0x0000	; ptl
dd 0x0100	; esp0
dd 0x00b0	; ss0
dd 0x0100	; esp1
...
gdt_start:
dd 0x00000000, 0x00000000	;0x00  	null
dd 0xa000bfff, 0x004f9b00	;0x08	default code
dd 0xb000ffff, 0x00409200	;0x10	default data

dd 0xe0000100, 0x00409200	;0x18	data1 	dpl 0
dd 0xe1000100, 0x0040b200	;0x20	data2 	dpl 1
dd 0xe2000100, 0x0040d200	;0x28	data3 	dpl 2
dd 0xe3000100, 0x0040f200	;0x30	data4	dpl 3

;;stacks
dd 0xc0000100, 0x00409200	;0x38	dpl 0
dd 0xc1000100, 0x0040b200	;0x40	dpl 1
dd 0xc2000100, 0x0040d200	;0x48	dpl 2
dd 0xc3000100, 0x0040f200	;0x50	dpl 3

dd 0xd10000ff, 0x0000e200	;0x58	ldt
dd 0xda680067, 0x00408900	;0x60	

dd 0xb0000070, 0x0040e900	;0x68	TSS 	dpl3
dd 0xb1000070, 0x0040e900	;0x70	TSS 	dpl3
dd 0xb2000070, 0x0040e900	;0x78	TSS 	dpl3
dd 0xb3000070, 0x0040e900	;0x80	TSS 	dpl3
dd 0xb4000070, 0x0040e900	;0x88	TSS 	dpl3

;;code segments
dd 0xa2000200, 0x0040ba00	;0x90	dpl 1
dd 0xa4000200, 0x0040da00	;0x98	dpl 2
dd 0xa6000200, 0x0040fa00	;0xa0	dpl 3
dd 0xa8000200, 0x00409e00	;0xa8	dpl 0

dd 0xc4000100, 0x00409200	;0xb0	stack for interrupt handler pl0
dd 0x00080100, 0x0000ec00	;0xb8	call gate descriptor pl 3
dd 0x00680000, 0x0000e500	;0xc0	task gate descriptor 

align 0x100    ;address = 0xd100
 ;Start of My LDT
ldt_start:
 ;Null Descriptor
 dd 0x0
